...
public class AppListFragment extends AttachListFragment implements DeviceConstants,
    ...
    public View onCreateView(LayoutInflater inflater, ViewGroup container,
        ...
        mAppList = findById(rootView, R.id.app_list);
    ...
    public boolean onBackPressed() {
        if (mDetailsShowing) {
            animateX(mAppDetails, 500, mAppIcon.getWidth() +
    ...
    private void refreshAppDetails() {
        ...
        if (!mDetailsShowing) {
            ...
            animateX(mAppDetails, 500, mAppDetails.getWidth(), mAppIcon.getWidth() +
                    2 * getResources().getDimensionPixelSize(R.dimen.app_margin));
            mDetailsShowing = true;
        }
    }

    private void animateX(final View v, final int duration, final int start, final int end) {
    ...
    private class LoadApps extends AsyncTask<Void, Void, List<AppItem>> {
        ...
        protected void onPostExecute(final List<AppItem> appItems) {
            if (appItems != null && isAdded()) {
                ...
                animateX(mAppDetails, 0, 0, mAppDetails.getWidth());
            }
        }
    }
