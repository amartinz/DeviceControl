    ...
    private void createServer() {
        ...
        mServer.get("/files/(?s).*", new HttpServerRequestCallback() {
            @Override public void onRequest(final AsyncHttpServerRequest req,
                ...
                final boolean useAuth = PreferenceHelper.getBoolean("wfm_auth", true);
                boolean isAuth = !useAuth;
                if (req.getHeaders().hasAuthorization() && useAuth) {
                    final String auth = req.getHeaders().getHeaders().get("Authorization");
                    if (auth != null && !auth.isEmpty()) {
                        final String[] parts = new String(Base64.decode(
                                auth.replace("Basic", "").trim(), Base64.DEFAULT)).split(":");
                        final String username = PreferenceHelper.getString("wfm_username", "root");
                        final String password = PreferenceHelper.getString("wfm_password", "toor");
                        isAuth = parts[0] != null && parts[0].equals(username)
                                && parts[1] != null && parts[1].equals(password);
                    }
                }
                if (!isAuth) {
