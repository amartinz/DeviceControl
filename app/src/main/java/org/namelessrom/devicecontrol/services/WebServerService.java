    ...
    private void createServer() {
        ...
        mServer.get("/files/(?s).*", new HttpServerRequestCallback() {
            @Override public void onRequest(final AsyncHttpServerRequest request,
                ...
                final String filePath = URLDecoder.decode(request.getPath().replace("/files", ""));
                ...
                if (isDirectory) {
                    ...
                    for (final File f : directories) {
                        sb.append("Directory: <a href=\"/files")
                                .append(Html.escapeHtml(f.getAbsolutePath().replace(sdRoot, "")))
                                .append("\">")
                    ...
                    response.send(HtmlHelper.getHtmlContainer("File Manager", sb.toString()));
                } else {
