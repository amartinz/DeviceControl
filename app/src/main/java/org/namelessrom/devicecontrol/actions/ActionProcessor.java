...
import org.namelessrom.devicecontrol.actions.extras.KsmDeferredAction;
import org.namelessrom.devicecontrol.actions.extras.KsmEnableAction;
import org.namelessrom.devicecontrol.actions.extras.KsmPagesAction;
import org.namelessrom.devicecontrol.actions.extras.KsmSleepAction;
...
public class ActionProcessor implements Constants {
    ...
    public static int getImageForCategory(final String category) {
        if (TextUtils.equals(CATEGORY_CPU, category)) {
            return R.drawable.ic_menu_perf_cpu;
        }
        if (TextUtils.equals(CATEGORY_GPU, category)) {
            return R.drawable.ic_menu_perf_gpu;
        }
        if (TextUtils.equals(CATEGORY_EXTRAS, category)) {
            return R.drawable.ic_menu_perf_extras;
        }
        if (TextUtils.equals(CATEGORY_FS, category)) {
            return R.drawable.ic_general_storage;
        }
        return 0;
    }
    ...
    public static ArrayList<Entry> getActions(final String category) {
        ...
        // GPU
        if (TextUtils.equals(CATEGORY_GPU, category) && Utils.fileExists(GPU_FREQUENCIES_FILE)) {
    ...
    public static ArrayList<Entry> getValues(final String action) {
        ...
        // GPU 3D scaling
        if (TextUtils.equals(ACTION_3D_SCALING, action)) addValuesOnOff(values);

        // Filesystem
        if (TextUtils.equals(ACTION_IO_SCHEDULER, action)) {
            final String[] scheds = IoSchedulerUtils.get().getAvailableIoSchedulers();
