...
public class MaterialTab implements View.OnTouchListener {
    ...
    public void setIconColor(int color) {
        this.iconColor = color;
        if (this.icon != null) { this.icon.setColorFilter(color); }
    }
    ...
    public void disableTab() {
        ...
        if (text != null) {
            this.text.setTextColor(Color.argb(0x99, Color.red(textColor), Color.green(textColor),
                    Color.blue(textColor)));
        }
        // set 60% alpha to icon
        if (icon != null) { setIconAlpha(0x99); }
        ...
        if (listener != null) { listener.onTabUnselected(this); }
    }

    public void activateTab() {
        // set full color text
        if (text != null) { this.text.setTextColor(textColor); }
        // set 100% alpha to icon
        if (icon != null) { setIconAlpha(0xFF); }
    ...
    public boolean onTouch(View v, MotionEvent event) {
        ...
        if (event.getAction() == MotionEvent.ACTION_UP) {
            ...
            this.background.reveal(lastTouchedPoint.x, lastTouchedPoint.y,
                    Color.argb(0x80, Color.red(accentColor), Color.green(accentColor),
                    ...
                    new Animator.AnimatorListener() {
                        ...
                        public void onAnimationEnd(Animator animation) {
                            background.reveal(lastTouchedPoint.x, lastTouchedPoint.y,
                                    primaryColor, 0, HIDE_DURATION, null);
                        }
    ...
    private void setIconAlpha(int paramInt) {
        ...
        this.icon.setColorFilter(
                Color.argb(paramInt, Color.red(this.iconColor), Color.green(this.iconColor),
