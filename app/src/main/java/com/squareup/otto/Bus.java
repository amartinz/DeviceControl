 ...
 * traditional Android in-process event distribution using explicit registration or listeners. It
 * is <em>not</em> a
 ...
 * event is <em>assignable.</em>  This includes implemented interfaces, all superclasses,
 * and all interfaces implemented
 * by superclasses.
 * <p/>
 * <p>When {@code post} is called, all registered handlers for an event are run in sequence,
 * so handlers should be
 * reasonably quick.  If an event may trigger an extended process (such as a database load),
 * spawn a thread or queue it
 * for later.
 * <p/>
 ...
 * re-throw it.
 * <p/>
 * <p>The Bus by default enforces that all interactions occur on the main thread.  You can
 ...
 * give the system a
 * second chance to handle dead events, they are wrapped in an instance of {@link com.squareup
 * .otto.DeadEvent} and
 * reposted.
 * <p/>
...
public class Bus {
     ...
     *
     * @param identifier a brief name for this bus, for debugging purposes.  Should be a valid
     *                   Java identifier.
     */
     ...
     * provide events.
     * <p/>
     * If any subscribers are registering for types which already have a producer they will be
     * called immediately
     * with the result of calling that producer.
     * <p/>
     * If any producers are registering for types which already have subscribers,
     * each subscriber will be called with
    ...
    public void register(Object object) {
        ...
        Map<Class<?>, Set<EventHandler>> foundHandlersMap =
                handlerFinder.findAllSubscribers(object);
     ...
     * <p/>
     * <p>If no handlers have been subscribed for {@code event}'s class,
     * and {@code event} is not already a
     ...
     * new events may be posted to the end of
     * the queue.
     */
    protected void dispatchQueuedEvents() {
        // don't dispatch if we're already dispatching, that would allow reentrancy and
        // out-of-order events. Instead, leave
     ...
     * java.lang.reflect.InvocationTargetException}. If the specified {@link java.lang.reflect
     * .InvocationTargetException} does not have a
