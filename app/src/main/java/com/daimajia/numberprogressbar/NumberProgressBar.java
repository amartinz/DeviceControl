 ...
 */
public class NumberProgressBar extends View implements ProgressCallback {
    ...
    @Override public void onProgress(final long downloaded, final long total) {
        final int progress;
        if (total > 0) {
            progress = (int) ((double) downloaded / (double) total * 100f);
        } else {
            progress = 0;
        }
        post(new Runnable() {
    ...
    public NumberProgressBar(Context context, AttributeSet attrs, int defStyleAttr) {
        ...
        final int defaultColor = Application.get().getAccentColor();
        ...
        mTextColor = attributes.getColor(R.styleable.NumberProgressBar_progress_text_color,defaultColor);
