...
public class MaterialMenuDrawable extends Drawable implements Animatable {
    ...
    public MaterialMenuDrawable(Context context, int color, int scale, int transformDuration,
            int pressedDuration) {
    ...
    @Override public void draw(Canvas canvas) {
        ...
        final float ratio =
                transformationValue <= 1 ? transformationValue : 2 - transformationValue;
    ...
    private void drawBottomLine(Canvas canvas, float ratio) {
        ...
        switch (animationState) {
            ...
            case X_CHECK:
                ...
                pivotX =
                        sidePadding + dip4 + (width / 2 - strokeWidth - sidePadding - dip4) * ratio;
                pivotY = height - topPadding - dip3 +
                        (topPadding + dip3 + height / 2 - strokeWidth - height) * ratio;
    ...
    private boolean resolveTransformation() {
        ...
        throw new IllegalStateException(
                String.format("Animating from %s to %s is not supported", currentIconState,
                        animatingIconState)
        );
    }

    @Override public void start() {
        if (transformationRunning || animatingIconState == null) return;
        if ((!drawTouchCircle || neverDrawTouch) && animatingIconState == currentIconState) {
            return;
        }

        if (animatingIconState != currentIconState) {
