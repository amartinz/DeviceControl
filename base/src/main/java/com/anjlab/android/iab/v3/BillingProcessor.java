...
public class BillingProcessor extends BillingBase {
    ...
    private ServiceConnection serviceConnection = new ServiceConnection() {
        ...
        public void onServiceConnected(ComponentName name, IBinder service) {
            ...
            if (!isPurchaseHistoryRestored() && loadOwnedPurchasesFromGoogle()) {
                ...
                if (eventHandler != null) { eventHandler.onPurchaseHistoryRestored(); }
            }
            if (eventHandler != null) { eventHandler.onBillingInitialized(); }
        }
    };
    ...
    private boolean loadPurchasesByType(String type, BillingCache cacheStorage) {
        ...
        try {
            Bundle bundle = billingService
                    .getPurchases(Constants.GOOGLE_API_VERSION, contextPackageName, type, null);
            if (bundle.getInt(Constants.RESPONSE_CODE) == Constants.BILLING_RESPONSE_RESULT_OK) {
                ...
                for (int i = 0; i < purchaseList.size(); i++) {
                    ...
                    String signature =
                            signatureList != null && signatureList.size() > i ? signatureList
                                    .get(i) : null;
    ...
    private boolean purchase(Activity activity, String productId, String purchaseType) {
        ...
        try {
            ...
            Bundle bundle = billingService
                    .getBuyIntent(Constants.GOOGLE_API_VERSION, contextPackageName, productId,
            ...
            if (bundle != null) {
                ...
                if (response == Constants.BILLING_RESPONSE_RESULT_OK) {
                    ...
                    eventHandler
                            .onBillingError(Constants.BILLING_ERROR_FAILED_TO_INITIALIZE_PURCHASE,
                                    null);
                }
            }
            return true;
    ...
    public boolean consumePurchase(String productId) {
        ...
        try {
            TransactionDetails transactionDetails =
                    getPurchaseTransactionDetails(productId, cachedProducts);
            if (transactionDetails != null &&
                    !TextUtils.isEmpty(transactionDetails.purchaseToken)) {
                int response = billingService
                        .consumePurchase(Constants.GOOGLE_API_VERSION, contextPackageName,
                                transactionDetails.purchaseToken);
                if (response == Constants.BILLING_RESPONSE_RESULT_OK) {
                    cachedProducts.remove(productId);
                    Log.d(LOG_TAG, "Successfully consumed " + productId + " purchase.");
                    return true;
                } else {
                    if (eventHandler != null) { eventHandler.onBillingError(response, null); }
                    Log.e(LOG_TAG,
                            String.format("Failed to consume %s: error %d", productId, response));
                }
            }
        } catch (Exception e) {
            Log.e(LOG_TAG, e.toString());
    ...
    private SkuDetails getSkuDetails(String productId, String purchaseType) {
        if (billingService != null) {
            try {
                ...
                Bundle skuDetails = billingService
                        .getSkuDetails(Constants.GOOGLE_API_VERSION, contextPackageName,
                                purchaseType, products);
                int response = skuDetails.getInt(Constants.RESPONSE_CODE);
                if (response == Constants.BILLING_RESPONSE_RESULT_OK) {
                    for (String responseLine : skuDetails
                            .getStringArrayList(Constants.DETAILS_LIST)) {
                        JSONObject object = new JSONObject(responseLine);
                        String responseProductId = object.getString(Constants.RESPONSE_PRODUCT_ID);
                        if (productId.equals(responseProductId)) { return new SkuDetails(object); }
                    }
                } else {
                    if (eventHandler != null) { eventHandler.onBillingError(response, null); }
                    Log.e(LOG_TAG,
                            String.format("Failed to retrieve info for %s: error %d", productId,
                                    response));
                }
            } catch (Exception e) {
    ...
    public boolean handleActivityResult(int requestCode, int resultCode, Intent data) {
        ...
        int responseCode =
                data.getIntExtra(Constants.RESPONSE_CODE, Constants.BILLING_RESPONSE_RESULT_OK);
        Log.d(LOG_TAG,
                String.format("resultCode = %d, responseCode = %d", resultCode, responseCode));
        ...
        if (resultCode == Activity.RESULT_OK &&
            ...
            try {
                ...
                if (purchasePayload.equals(developerPayload)) {
                    if (verifyPurchaseSignature(productId, purchaseData, dataSignature)) {
                        ...
                        if (eventHandler != null) {
                            eventHandler.onProductPurchased(productId, new TransactionDetails(
                                    new PurchaseInfo(purchaseData, dataSignature)));
                        }
                    } else {
                        Log.e(LOG_TAG, "Public key signature doesn't match!");
                        if (eventHandler != null) {
                            eventHandler.onBillingError(Constants.BILLING_ERROR_INVALID_SIGNATURE,
                                    null);
                        }
                    }
                } else {
                    Log.e(LOG_TAG, String.format("Payload mismatch: %s != %s", purchasePayload,
                            developerPayload));
                    if (eventHandler != null) {
                        eventHandler
            ...
            if (eventHandler != null) { eventHandler.onBillingError(responseCode, null); }
        }
        return true;
    }
