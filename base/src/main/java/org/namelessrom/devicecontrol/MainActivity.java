...
public class MainActivity extends BaseActivity implements DeviceConstants,
    ...
    @Override protected void onCreate(Bundle savedInstanceState) {
        ...
        final MenuListArrayAdapter mAdapter = new MenuListArrayAdapter(
                this, R.layout.menu_main_list_item, MENU_ENTRIES, MENU_ENTRY_VALUES, MENU_ICONS);
    ...
    @Override public void onItemClick(AdapterView<?> adapterView, View view, int i, long l) {
        loadFragmentPrivate((String) adapterView.getItemAtPosition(i), false);
    }
    ...
    public static void loadFragment(final Activity activity, final String id) {
        loadFragment(activity, id, false);
    }

    public static void loadFragment(final Activity act, final String id, final boolean onResume) {
        if (act instanceof MainActivity) {
            ((MainActivity) act).loadFragmentPrivate(id, onResume);
        }
    }

    private void loadFragmentPrivate(final String title, final boolean onResume) {
        switch (title) {
            ...
            case ID_PERFORMANCE_INFO:
                ...
                break;
            //--------------------------------------------------------------------------------------
            case ID_CPU_SETTINGS:
                if (!onResume) mCurrentFragment = new CpuSettingsFragment();
                mTitle = mFragmentTitle = R.string.cpu_settings;
                mSubFragmentTitle = -1;
                break;
            case ID_GOVERNOR_TUNABLE:
                ...
                break;
            //--------------------------------------------------------------------------------------
            case ID_GPU_SETTINGS:
                if (!onResume) mCurrentFragment = new GpuSettingsFragment();
                mTitle = mFragmentTitle = R.string.gpu_settings;
                mSubFragmentTitle = -1;
                break;
            //--------------------------------------------------------------------------------------
