package org.namelessrom.devicecontrol.modules.tools.appmanager.permissions;
...
public class PermissionItemView extends LinearLayout implements View.OnClickListener {
    ...
    public void setPermission(AppSecurityPermissions.MyPermissionGroupInfo grp,
        ...
        if (first) {
            ...
            if (icon != null) {
                icon = icon.mutate();
                icon.setColorFilter(new LightingColorFilter(Color.BLACK, ThemeHelper.with(getContext()).getAccentColor()));
            }
        }
